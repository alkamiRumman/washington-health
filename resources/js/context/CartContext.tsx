...
import { Cart, CartType, Equipment } from '@/types';
...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const updateCart = (updatedCart: Cart) => {
        setCarts((prevCarts) => prevCarts.map((cart) => (cart.id === updatedCart.id ? updatedCart : cart)));
    };
                ...
                const updatedDrawers = cart.drawers.map((drawer) => {
                    ...
                    return {
                        ...
                        equipment: drawer.equipment.map((equipment) =>
                            equipment.id === updatedEquipment.id ? { ...updatedEquipment, lastUpdated: format(new Date(), 'yyyy-MM-dd') } : equipment,
                        ),
                    };
                });
    ...
    const updateLockStatus = (cartId: string, lockType: 'mediLock' | 'supplyLock', status: boolean) => {
        setCarts((prevCarts) =>
            prevCarts.map((cart) => (cart.id === cartId ? { ...cart, [lockType]: status, lastChecked: format(new Date(), 'yyyy-MM-dd') } : cart)),
        );
    };
